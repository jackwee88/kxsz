  ...
  <view>
    <scroll-view scroll-y>
      <view class="recommend-item">
        ...
        <view class="item-middle">
          ...
          <view class="middle-image">
            <view v-for="(item, index) in studyDetails.picture_arr" :key="index">
              <image :src="item" style="width: 220rpx;height: 220rpx;" />
            </view>
          </view>
        </view>
        <view class="smallicon">
          <view class="smallicon-flex" @tap="praise">
            <image
              class="collecticon"
              v-if="studyDetails.is_give"
              src="../../static/index/collect.png"
              style="width: 45rpx;height: 42rpx;"
            />
            <image
              class="collecticon"
              v-else
          ...
          <view class="smallicon-flex">
            <image
              src="../../static/index/messageicon.png"
              mode="widthFix"
      ...
      <view class="input-box">
        ...
        <input placeholder="请输入评论内容" class="input" v-model="comment_content" />
...
export default {
  ...
  onLoad(event) {
    console.log(event);
    // this.getData()
    const { dy_id, browse_times, comment_count, thumbs_times } = JSON.parse(
      decodeURIComponent(event.pulishedDetail)
    );
    if (dy_id) {
      this.dy_id = dy_id;
        ...
        success: res => {
          if (this.count < this.page) {
            ...
            (this.page = this.page + 1),
              ...
              (this.release = release.concat(list)),
            ...
            success: res => {
              ...
              if (prevPages) {
                if (prevPages.__route__ == "pages/index/index") {
                  ...
                  prevPages.__route__ == "pages/studySquare/studySquare"
                ) {
                  var up = "studylist[" + index + "].comment_count";
                } // prevPages.setData({
                //   [up]: newp
