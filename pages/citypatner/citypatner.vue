...
<view class="big-box" v-if="if_over">
  <view class="info-box">
    ...
    <view class="input">
      ...
      <input type="text" :value="username" data-type="username" @input="inputChange"></input>
    </view>
    <view class="input">
      ...
      <picker mode="region"  @click="toggleTab" :value="region" >
        ...
        <view class="picker clear" v-if="region != ''" style="float:right">
         ...
         <view class="picker" v-else>
          <text style="color:#595959;padding-left:10rpx">请选择省市县</text><image src="/static/img/freeTeaching/arrow-rt.png" style="float:right;width:20rpx;height:30rpx;position:relative;top:10rpx;right:10rpx"></image>
        </view>
      </picker>
    </view>
    <view class="input" style="claer:both;margin-top:20rpx;">
      <text class="text">邮箱：</text>
      <input class="text" :value="email" data-type="email" @input="inputChange"></input>
    </view>

    <textarea placeholder="请输入您的留言内容" style="width:418rpx;height:213rpx;" data-type="content" @input="inputChange" :value="content"></textarea>
  ...
  methods: {
   ...
   toggleTab() {
    ...
    submit(e) {
      ...
      util.ajax({url:'/partner/upload',data:obj, success:(res) => {
        uni.showToast({
          title: result.msg
        });
        that.close();
