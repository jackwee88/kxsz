...
<view class="newaddress">
  ...
  <view class="form">
    ...
    <view class="detailaddress form_item">
      <label>详细地址:</label>
      <input class="dizhiinput input" @input="inputChange" data-value="detailedAddress" placeholder="街道，楼牌号" :value="detailedAddress"></input>
    </view>
    </view>
  </form>
...
<script>
// pages/onlinestore/newaddress/newaddress.js
...
export default {
  ...
  onLoad: function (options) {
    ...
    if (options.list != undefined) {
      ...
      util.ajax('/api/address/onelist', {
        ar_id: ar_id
      }, res => {
        that.setData({
          name: res.data.person,
          tel: res.data.mobile,
          region: [res.data.province, res.data.city, res.data.area],
          detailedAddress: res.data.address,
          selected: res.data.default
        });
      });
    }
  ...
  methods: {
    ...
    save: function () {
      ...
      if (this.type == 1) {
        util.ajax('/api/address/edit', param, res => {
          if (res.status == 1) {
            wx.showToast({
              title: '修改成功',
              duration: 2000,
            ...
            if (that.selected == true) {
              util.ajax('/api/goods/OrderMoney', params, reslute => {
                ...
                if (prevPages) {
                  if (prevPages.__route__ == 'pages/onlinestore/sureorder/sureorder') {
                    ...
                    prevPages.setData({
                      address: obj,
                      totals: reslute.data.amount.toFixed(2),
                      transport_total: reslute.data.transport,
                      s: 1
                    }); //设置数据
                  }
                }
        ...
        util.ajax('/api/address/add', param, res => {
          if (res.status == 1) {
            wx.showToast({
              title: '添加成功',
              duration: 2000
