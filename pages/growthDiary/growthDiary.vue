	...
	<view>
		<view class="growthDiary">
			<view class="growthDiary-top">
				<view class="topLeft topcont"><image class="usertoux" :src="user.avatar" /></view>
				...
				<view class="Signature">
					<view class="Signature_title">
						<image class="moduleicon" src="/static/img/growthDiary/qianming.png" />
						个性签名
					</view>
						...
						<view class="studyitem-middle">
							...
							<view class="thirdlineblock">
								<view class="thirdline" :data-dy_id="item.dy_id" :data-index="index" @tap="praise">
									...
									<image class="collecticon" v-if="item.is_give == false" src="../../static/index/uncollect.png" style="margin-top:6rpx;" />
		...
		<view class="canvas" v-if="isCan">
			...
			<view class="button" style="position:fixed;bottom:10%;left:0;right:0;align-items:center;">
				<button class="share l" :data-dy_id="dy_id" style="padding-left:0;width:200rpx" open-type="share" data-hb="1">
					<image src="/static/img/index/wx.png" class="share" style="font-size;28rpx;margin-top:0;width:40rpx;height:40rpx;vertical-align:middle" />
					<text class="shares">分享</text>
				</button>
				<image class="close-can" @tap.stop="closeCan" style="height:50rpx;width:50rpx" src="/static/img/my/close.png" />
				<button class="r" @tap="save">
					<image src="/static/img/index/xc.png" class="share" style="font-size;28rpx;vertical-align:middle;width:26rpx;height:26rpx;" />
					保存至相册
				</button>
			</view>
		</view>
	...
	onShareAppMessage: function(e) {
		...
		if (e.from === 'button') {
			if ((e.target.dataset.hb = 1)) {
				...
				return {
					title: title,
					path: '/pages/growthDiary/growthDiary?uid=' + that.uid + '&is_share=1',
					imageUrl: img,
					success: function(res) {}
				};
			}
			...
			util.ajaxs(
				...
				res => {
					...
					if (res.data.is_ok == true) {
						(this.daka[index].is_give = !daka[index].is_give), (this.daka[index].thumbs_times = daka[index].thumbs_times + 1), (this.is_ok = 'y');
					} else {
						(this.daka[index].is_give = !daka[index].is_give), (this.daka[index].thumbs_times = daka[index].thumbs_times - 1), (this.is_ok = 'n');
					}
				}
			);
			setTimeout(function() {
