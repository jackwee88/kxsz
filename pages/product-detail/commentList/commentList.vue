...
<view :class="'evaluate ' + evaluateHeight">
  <text v-for="(item, index) in LaberList" :key="index" :class="'LaberList ' + (index== indexId ? 'checkLaber' : '')" :data-id="item.name" :data-index="index" @tap="changeBtn">{{item.name}}{{item.number=='' ? '' : '('+item.number+')' }}</text>
</view>
<view class="showImg" @tap="showHeight">
  <image v-if="evaluateHeight!=''" src="/static/img/commentList/bottom.png" class="topImg" mode="widthFix"></image>
  <image v-if="evaluateHeight==''" src="/static/img/commentList/top.png" class="topImg" mode="widthFix"></image>
</view>
<!-- 评价主内容 -->
<view class="evaluateContent">
  <view class="evaluateList" v-for="(item, index) in evaluateList" :key="index">
    <view class="evaluateValue">
      ...
      <!-- 这个是固定的logo -->
      <image src="https://kxsx-front-pic.oss-cn-beijing.aliyuncs.com/kxsx/%E5%BE%AE%E4%BF%A1%E5%9B%BE%E7%89%87_20191031134814.png" class="userImg"></image>
    ...
    <view class="evaluateDetailsImg">
      ...
      <view class="img-box">
        ...
        <view class="videoUrlImg" v-if="item.videoUrl">
          <image v-if="item.videoUrl" style="width:90%;height:200rpx;margin-bottom:0rpx;border-radius: 10rpx;" :hidden="item.fullScreen" class="case-picture" :src="item.videoUrl + '?spm=a2c4g.11186623.2.1.yjOb8V&x-oss-process=video/snapshot,t_0000,f_jpg,w_800,h_600,m_fast'" mode="widthFix"></image>
          <image v-if="item.videoUrl" :data-fullScreen="item.fullScreen" class="bt" mode="scaleToFill" :data-src="item.videoUrl" :data-index="index" @tap="playVideo" src="/static/img/index/play3.png" style="width:60rpx;height:60rpx;position:absolute;top:0;left:0;right:0;bottom:0;margin:auto;"></image>
        </view>
        <!-- 图片 -->
        <view class="evaluateDetailsImgList" v-for="(items, index2) in item.image_array" :key="index2" v-if="items">
          <image :src="items" @tap="previewImg" :data-effect_pic="item.image_array" :data-src="items" mode="aspectFill" class="acticleimg"></image>
        </view>
<view style="clear:both"></view>
      </view>
      ...
      <!-- <view class="evaluateDetailsImgList">
        <image src="https://pic.kaifadanao.cn/20191029/8e2f59aaeb0dbbc02eae5bc64e5cfa5e.jpg" mode="widthFix" bindtap='previewImg' data-src="https://pic.kaifadanao.cn/20191029/8e2f59aaeb0dbbc02eae5bc64e5cfa5e.jpg" data-effect_pic='{{imgList}}'></image>
      </view> -->
...
export default {
  data() {
    return {
      ...
      evaluateList: '',
      //评论列表
      imgList: ['https://pic.kaifadanao.cn/20191029/8e2f59aaeb0dbbc02eae5bc64e5cfa5e.jpg', 'https://pic.kaifadanao.cn/20191029/8e2f59aaeb0dbbc02eae5bc64e5cfa5e.jpg'],
      //评论的图片列表
      collect_p: false,
      //收藏商品标识
      p_id: 0,
  ...
  onLoad: function (options) {
    ...
    util.ajax('/api/evaluate/getEvaluateHeadLableList', {
      goods_id: that.p_id
    }, res => {
      that.setData({
  ...
  methods: {
    ...
    buyNow() {
      util.ajax('/api/index/getProfile', '', res => {
        ...
        if (!wx.getStorageSync("token")) {
          ...
          wx.navigateTo({
            url: '/pages/onlinestore/sureorder/sureorder?type=1&ids=' + that.p_id + '&now_buy=1'
          });
        }
      });
    },
    ...
    checkedBtn: function (e) {
      ...
      let detailValue = this.obtnArry.filter(it => it.selected).map(it => it.name);
    },
    //标签点击选择(单选)
