<template>
  <view>
      ...
      <block v-for="(item, index) in goods" :key="index">
        <view class="store">
          <view class="box">
            <view class="store_left">
              <image
                class="storeimg"
                :src="item.goods_sku.spec_image ? item.goods_sku.spec_image : item.image"
        ...
        <view v-if="item.video.length != 0">
          <view class="store" v-for="(items, index2) in item.video" :key="index2">
            <view class>
              <view class="store_left">
                <image class="storeimg" :src="items.thumb" />
              </view>
              <view class="store-flex">
                <view>
        ...
        <view v-if="item.bindpacks.length != 0">
          <view class="store" v-for="(items_, index2) in item.bindpacks" :key="index2">
            <view class>
              <view class="store_left">
                <image class="storeimg" :src="items_.icon" />
              </view>
              <view class="store-flex">
                <view>
    ...
    <view class="content">    
      <image class='storeimg item-img' src='{{item.image}}'>
     ...
     <view>
      ...
      <view class="address-content" @tap="linkToMy" :data-money="totals">
        ...
        <view class="address-right">
          <image src="/static/img/freeTeaching/arrow-rt.png" />
        </view>
      </view>
      <!-- 地址标题 -->
      <view class="title-w">
      ...
      <view class="payfangshi">
        ...
        <view class="wechatzhifu">
          <image class="wechaticon" src="/static/img/onlinestore/wechaticon.png" />
          <text>微信支付</text>
          <!-- 判断是否选中状态 -->
          <view class="rodio">
      ...
      <view class="payfangshi jf">
        <!-- <view class='payfangshi_title'>支付方式</view> -->
        <view class="wechatzhifu">
          <image class="wechaticon" src="/static/img/onlinestore/huiyuanjifen.png" />
          <text>可用积分{{ integral }}</text>
          <!-- 判断是否选中状态 -->
          <view class="rodio">
            <icon
              v-if="selected_integral"
              type="success"
              color="#48DB8D"
              size="20"
              @tap="selectList"
        ...
        >
          ...
          <view style="display:flex;align-items:center;height:90rpx">
            ...
            <image src="/static/img/freeTeaching/arrow-rt.png" style="width:20rpx;height:24rpx;" />
          </view>
        </view>
      ...
      <view
        class="wechatzhifu"
        style="border-top:1rpx solid #ccc;margin-top:36rpx;text-align:center"
      >
...
export default {
  data() {
    return {
      ...
      integral: "109",
      storeimg:
        "https://kxsx-front-pic.oss-cn-beijing.aliyuncs.com/test/chanpintu.png",
      title: "多功能尺子教学全套课程",
      guankan: "永久观看",
      selected: true,
  ...
  onLoad: function(event) {
    ...
    if (this.banner.now_buy == 1) {
      ...
      this.assemble = this.banner;
      this.goods_sku_id = this.banner.goods_sku_id;
      this.goods_num = this.banner.goods_num;
      this.buy_num = this.banner.goods_num;
      this.goods_spec_id = this.banner.goods_spec_id;
      this.p_id = this.banner.ids;
    }
    var that = this;
    this.dzxyFun();
    ...
    pay: function() {
      ...
      if (
        ...
        let param = {
          ...
          quantity: 1,
          type: 2,
          assemble_order_id: this.assemble_order_id
        };
        ajax({
    ...
    selectList: function(e) {
      ...
      if (this.selected_integral == true) {
        ...
        if (this.flag_n == false) {
          var new_total = (
            parseFloat(this.totals) + parseFloat(integral)
          ).toFixed(2);
          this.jf();
        }
